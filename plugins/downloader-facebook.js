...
const handler = async (m, {conn, args, command, usedPrefix}) => {
  if (!args[0]) throw `*[❗𝐈𝐍𝐅𝐎❗] Enter a Facebook link, example: ${usedPrefix + command}* https://fb.watch/OTpgn6UFQ/`;
  if (!args[0].match(/www.facebook.com|fb.watch/g)) throw `*[❗𝐈𝐍𝐅𝐎❗] Enter a Facebook link, example: ${usedPrefix + command}* https://fb.watch/OTpgn6UFQ/`;
  try {
    await m.reply(`*[❗] Downloading your video, please wait a moment, This process can take between 2 and 10 minutes depending on the length of the video....*`);
    const d2ata = await facebook.v1(args[0]);
    let r2es = '';
    if (d2ata.urls && d2ata.urls.length > 0) {
      r2es = `${d2ata.urls[0]?.hd || d2ata.urls[1]?.sd || ''}`;
    }
    conn.sendFile(m.chat, r2es, 'error.mp4', `*Here is your video*`, m);
  } catch (err1) {
    try {
          ...
          try {
            ...
            try {
              ...
              try {
                ...
                throw `*[❗𝐈𝐍𝐅𝐎❗] 𝙴𝚁𝚁𝙾𝚁, Please Try Again*`;
              }
            }
          }
        }
