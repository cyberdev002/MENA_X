  ...
  if (lister.includes(feature)) {
    if (feature == 'create') {
      try {
        ...
        conn.dropmail[id] = [
          await m.reply('*EMAIL:*\n' + eml[0] + '\n\n' + '*ID:*\n' + eml[1] + '\n\n*Expired:*\n' + msToTime(timeDiff) + '\n\n_Example *' + usedPrefix + command + ' message* To check your inbox_'),
          eml[0],
          eml[1],
          eml[2],
        ];
    ...
    if (feature == 'message') {
      if (!conn.dropmail[id]) return m.reply('There are no messages, please create an email first\nExample *' + usedPrefix + command + ' create*');
...
*Download* : ${v.downloadUrl}
   `.trim();
        ...
        await m.reply(teks || '*Empty*' + '\n\n_Example *' + usedPrefix + command + ' delete* To delete emails_');
      } catch (e) {
        await m.reply(eror);
      }
    }
    if (feature == 'delete') {
      if (!conn.dropmail[id]) return m.reply('There is no valid email');

      try {
        delete conn.dropmail[id];
        await m.reply('Successfully deleted email');
      } catch (e) {
        await m.reply(eror);
      }
    }
