...
const handler = async (m, {conn, args, usedPrefix, command}) => {
  if (!args[0]) throw '`Give me Youtube Video Link`\n\n- Example - .yt https://youtu.be/iXfqn7alvFo?si=CHbcj-NdZ88yuCYo\n\n> ꜱɪᴍᴘʟᴇ ɪꜱ ʙᴇᴀᴜᴛʏ 🌙';
  let enviando;
  if (enviando) return  
      enviando = true    
  let youtubeLink = '';
    ...
    if (index >= 0) {
      if (Array.isArray(global.videoList) && global.videoList.length > 0) {
        ...
        if (matchingItem) {
          ...
          enviando = false  
          throw `*[❗] To be able to use the command in this way (${usedPrefix + command} <name>), Please search for videos with the command ${usedPrefix}playlist <text>*`;
        }
      } else {
        enviando = false  
        throw `*[❗] To be able to use the command in this way (${usedPrefix + command} <name>),Please search for videos with the command  ${usedPrefix}playlist <text>*`;
  ...
  const { key } = await m.reply(`\`Youre video processing...\`\n\n- ◉ ᴍɪᴢᴛʏ ʏᴛ ᴅᴏᴡɴʟᴏᴀᴅᴇʀ ◉\n\n> ꜱɪᴍᴘʟᴇ ɪꜱ ʙᴇᴀᴜᴛʏ 🌙`);
  try {
   ...
   if (fileSizeInMB > 100) {
    ...
    await conn.sendMessage(m.chat, {text: `*[ ✅ ] Video downloaded and sent successfully*\n\n*—◉ It was sent in document format because the video weighs${roundedFileSizeInMB} MB and exceeds the limit established by WhatsApp.*\n*◉ Title:* ${ttl_1}`, edit: key}, {quoted: m});
    enviando = false
   } else {
    await conn.sendMessage(m.chat, {video: buff, caption: `*▢ Title:* ${ttl_1}\n*▢ Video Size:* ${roundedFileSizeInMB} MB`, fileName: ttl_1 + '.mp4', mimetype: 'video/mp4'}, {quoted: m});
    await conn.sendMessage(m.chat, {text: `\`ʏᴏᴜʀᴇ ᴠɪᴅᴇᴏ ᴅᴏᴡɴʟᴏᴀᴅ ꜱᴜᴄᴄᴇꜱꜱꜰᴜʟʟ\`\n\n- ᴍɪᴢᴛʏ ʏᴛ ᴅᴏᴡɴʟᴏᴀᴅᴇʀ \n\n> ꜱɪᴍᴘʟᴇ ɪꜱ ʙᴇᴀᴜᴛʏ 🌙`, edit: key}, {quoted: m});
    enviando = false   
   }
 } catch (ee) {
    console.log(ee)
  try {
    ...
    await conn.sendMessage(m.chat, {text: '*ℹ️ ʏᴏᴜʀᴇ ᴠɪᴅᴇᴏ ᴅᴏᴡɴʟᴏᴀᴅ ꜱᴜᴄᴄᴇꜱꜱꜰᴜʟʟ*\n\n▢ ᴍɪᴢᴛʏ ʏᴛ ᴅᴏᴡɴʟᴏᴀᴅᴇʀ \n\n*───────────ꜱɪᴍᴘʟᴇ ɪꜱ ʙᴇᴀᴜᴛʏ 🌙───────────*', edit: key}, {quoted: m});
    enviando = false
  } catch (ee2) {
    console.log(ee2)
    try {
      const mediaa = await ytMp4(youtubeLink);
      await conn.sendMessage(m.chat, {video: {url: mediaa.result}, fileName: `error.mp4`, caption: `\`ᴍɪᴢᴛʏ ʏᴛ ᴅᴏᴡɴʟᴏᴀᴅᴇʀ\`\n\n> ꜱɪᴍᴘʟᴇ ɪꜱ ʙᴇᴀᴜᴛʏ 🌙`, thumbnail: mediaa.thumb, mimetype: 'video/mp4'}, {quoted: m});
      await conn.sendMessage(m.chat, {text: '`ʏᴏᴜʀᴇ ᴠɪᴅᴇᴏ ᴅᴏᴡɴʟᴏᴀᴅ ꜱᴜᴄᴄᴇꜱꜱꜰᴜʟʟ`\n\n- ᴍɪᴢᴛʏ ʏᴛ ᴅᴏᴡɴʟᴏᴀᴅᴇʀ \n\n> ꜱɪᴍᴘʟᴇ ɪꜱ ʙᴇᴀᴜᴛʏ 🌙', edit: key}, {quoted: m});
      enviando = false
    } catch {
      try {
        ...
        throw '*[❗] Error, could not download the video.*';
      }
    }
  }
}};
